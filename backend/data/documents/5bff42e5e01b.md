...
This article explains every step NeuroTalk takes from the moment sound hits your microphone to the moment you hear the agent's reply — and how interruption is handled in between. It also reflects the current product surface: an immersive voice orb, a dedicated reading mode, voice preview, speech-speed controls, dark/light themes, and a full-screen document workspace for long-form reading.

The two directions are covered separately: **client → server** (your voice going in) and **server → client** (the agent's voice coming back).
...
## The product layer on top of the pipeline

The raw audio pipeline is only half the story. The current frontend is built around a few product rules that shape how the technical system behaves:

### 1. Conversation mode is voice-first

The default workspace centers the voice orb, keeps the transcript close to the interaction, and treats the document panel as contextual support. This matches the real interaction priority: users talk first, then inspect context.

### 2. Reading mode is document-first

When the user switches to `Reading`, the document workspace expands to full width. The goal is focus: the document, the live reading state, and the word-level highlight become the primary experience.

### 3. Explicit reading and conversational resume are different

NeuroTalk now separates:

- explicit UI reading actions such as `Read aloud`
- conversational resume commands such as `continue`, `resume`, or `keep reading`

That distinction matters because users expect the visible document button to be deterministic. In the current behavior:

- `Read aloud` starts the selected document from the beginning
- conversational `continue` resumes from saved reading position

This removes ambiguity and makes the UI easier to trust.

### 4. Voice settings are part of the product, not a debug control

The voice settings modal now exposes:

- selectable TTS voices
- preview playback for the selected voice
- adjustable speech speed
- playback summary state

Speech speed is not just a frontend knob. It is sent through the active transport and applied in the backend TTS pipeline so previews and live reading use the same setting.

### 5. Dark and light themes both need deliberate design

The UI is not a dark-only product anymore. The light theme now has its own surface treatment, shell styling, and orb-stage treatment so it does not look like a dark interface with a white overlay dropped on top.
